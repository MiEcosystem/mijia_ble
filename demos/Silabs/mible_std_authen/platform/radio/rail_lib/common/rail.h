 ...
 * @return Status code indicating success of the function call.
 *
 * Although a RAIL handle is included for potential future expansion, it is
 * currently not used.
 *
 */
 ...
 *       If the multitimer is not needed, do not call this function to allow the
 *       code to be dead stripped.
 */
...
///     which can then use RAIL_GetRxPacketInfo() and RAIL_GetRxPacketDetailsAlt() to
...
///     as determined from RAIL_GetRxPacketInfo(). RAIL_GetRxPacketDetailsAlt()
...
///     to prevent a \ref RAIL_EVENT_TX_UNDERFLOW event from occurring.
...
///     will occur telling the application to consume some RX packet data to prevent a
///     \ref RAIL_EVENT_RX_FIFO_OVERFLOW event from occurring.
...
/// to know the size of each FIFO. The receive FIFO is internal to RAIL and its
/// default size is 512 bytes. The receive FIFO is level-based in that the \ref
...
/// application should empty enough of the FIFO to go under the threshold. To
/// defer reading the FIFO to main context, the application can disable or
/// re-enable the receive FIFO threshold event using RAIL_ConfigEvents() with
/// the mask \ref RAIL_EVENT_RX_FIFO_ALMOST_FULL.
///
...
///
/// In FIFO mode, FIFOs can store multiple packets. Depending on the
 ...
 * FIFO or returns zero if railHandle is NULL or if the TX FIFO is full.
 *
 ...
 *   occur. If too much data is put into the TX FIFO, the extra data will
 ...
 *   any CRC bytes, which the packet configuration causes to be sent
 *   automatically.
 *
 ...
 *   this function.
 * @param[in] initLength A number of initial bytes already in the TX FIFO.
 * @param[in] size A desired size of the TX FIFO in bytes.
 * @return Returns the FIFO size in bytes.
 *
 * This function sets the memory location for the TX FIFO. It
 ...
 * for previously-written memory to be set in the TX
 * FIFO.
 *
 ...
 *   been put into the TX FIFO, a \ref RAIL_EVENT_TX_UNDERFLOW event will
 *   occur. If too much data is put into the TX FIFO, the extra data
 ...
 *   the RX FIFO. This memory must persist until the next call to this function.
 * @param[in,out] size A desired size of the RX FIFO in bytes. This will be
 *   populated with the actual size during the function call.
 * @return Status code indicating success of the function call.
 *
 * This function sets the memory location for the RX FIFO. It
 ...
 * use RAIL_GetRxPacketInfo() instead.
 *
 * @param[in] railHandle A RAIL instance handle.
 ...
 *   integrity checking. Also note that the packet could be aborted, cancelled, or
...
/**
 * Gets detailed information about a ready packet received (one whose
 ...
 *   RAIL_HoldRxPacket(), or sentinel values \ref RAIL_RX_PACKET_HANDLE_OLDEST
 *   or \ref RAIL_RX_PACKET_HANDLE_NEWEST.
 * @param[out] pPacketDetails An application-provided non-NULL pointer to store
 *   \ref RAIL_RxPacketDetails_t for the requested packet. The timeReceived
 *   field packetTime will be populated with a timestamp corresponding to an
 *   arbitrary location in the packet. Call \ref RAIL_GetRxTimePreambleStart,
 *   \ref RAIL_GetRxTimeSyncWordEnd, or \ref RAIL_GetRxTimeFrameEnd to adjust
 *   that timestamp for different locations in the packet.
 * @return \ref RAIL_STATUS_NO_ERROR if pPacketDetails was filled in,
 *   or an appropriate error code otherwise.
 *
 ...
 *   rate greater than 500 kbps. If this is a requirement,
 *   filtering must currently be done by the application.
 *
 * @{
 */
