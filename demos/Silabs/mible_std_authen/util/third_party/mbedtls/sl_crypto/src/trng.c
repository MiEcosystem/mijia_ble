                       ...
                       unsigned char *output, size_t len, size_t *olen )
{
    ...
    while (len > 0)
    {
        ...
        /* Preliminary noise alarm is not critical, but will cause FIFO data to be thrown
         * away. We should do the same with the data we have already gotten out of the FIFO
         * but can return 0 to mbed TLS. */
        ...
        /* Critical alarm has been signaled so we throw the generated data away and signal the alarm
         * condition upwards. */
        if (ret != 0)
        {
            output_len = 0;
