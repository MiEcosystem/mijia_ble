...
Mijia_Gatt  gMibleGatt_db[MIBLE_GATT_HANDLE_MAX_NUM] = {0};
...
Mijia_Timer_db gMiJia_TimerPool[MIBLE_TIMER_MAX_NUM];
...
void mible_task_post_handler( uint32_t count )
{
	...
	else
	{
	   ...
	   CY_LOG_INFO("mible_task_post_handler:stopped\r\n");
	}
}
...
int mijia_gatt_write(uint16_t conn_id, void *ucReq )
{
    ...
    if (p_req->val_len == 0)
    {
        CY_LOG_INFO("mijia_gatt_write MI_ERR_INVALID_LENGTH1\r\n");
...
int mijia_gatt_read(uint16_t handle,uint8_t *readData,uint16_t *len,uint8_t offset)
{
	...
	if(i>=MIBLE_GATT_HANDLE_MAX_NUM)
	{
		...
		CY_LOG_INFO("Read gatt(XiaoMi) Not Found:len=%d, handle_val=0x%4x\r\n", *len, handle);
		return 0;
	}
	
    if((offset > gMibleGatt_db[i].handle_max_len))
    {
        CY_LOG_INFO("mijia_gatt_read: MI_ERR_INVALID_LENGTH2\r\n");
        return MI_ERR_INVALID_LENGTH;
    }
...
void advertising_start(void){
    ...
    if(MI_SUCCESS != mible_gap_adv_start(&adv_param)){
        CY_LOG_INFO("mible_gap_adv_start failed. \r\n");
        return;
    }
